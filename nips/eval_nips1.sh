...
for peft_name in sft_bnb-b2-a64-c1.0-lr4e-5-v2_4090_noidentity_nospecial_mmlu_guanaco_dolly-flashattn-samplepacking ; do
    python -m helm.benchmark.run \
    --conf-paths run_test.conf \
    --enable-local-huggingface-models /model/Llama-2-13b-hf \
    --suite nips \
    --max-eval-instances 200 \
    --model-args="{\"quantization_config\":{\"load_in_4bit\":true,\"bnb_4bit_use_double_quant\":true,\"bnb_4bit_quant_type\":\"nf4\",\"llm_int8_has_fp16_weight\":true,\"quant_method\":\"bitsandbytes\"},\"dtype\":\"float16\",\"peft\":\"/home/lzy/nips2023_llm_challenge/sft/outputs/$peft_name\",\"name_ext\":\"$peft_name\"}" \
    --num-threads 1
done
